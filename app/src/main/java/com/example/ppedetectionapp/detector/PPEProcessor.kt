...
class PPEProcessor {
    ...
    private fun boxContainsPoint(box: BoundingBox, point: Pair<Float, Float>): Boolean {
        // Dynamic margin: larger relative margin for smaller (distant) boxes
        val isDistant = (box.x2 - box.x1) < 0.25f
        val factorX = if (isDistant) 0.35f else 0.20f
        val factorY = if (isDistant) 0.30f else 0.15f
        
        val marginX = (box.x2 - box.x1) * factorX
        val marginY = (box.y2 - box.y1) * factorY
        ...
        fun update(workerResults: List<WorkerResult>): List<WorkerResult> {
            ...
            for (wr in sortedResults) {
                ...
                for (gearName in PPEConstants.PPE_PAIRS.keys) {
                    ...
                    // Use configurable vote threshold for better stability
                    val smoothedStatus = if (presentVotes >= slotHistory.size * PPEConstants.SMOOTH_VOTE_THRESHOLD) "present" else "absent"
